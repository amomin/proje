...
$cc_cards = array(
	...
	'o','o','o','o',
	'o','o','o','o',
	'o','o',CC_GOTOGO,CC_GOTOJAIL
);
$ch_cards = array(
	'o','o','o','o',
	'o','o',CH_GOTOGO,CH_GOTOJAIL,
...
for ($i=0;$i<$num_games;$i++){
	$c = simulation($board, $num_rolls);
	foreach ($count as $b=>$v){
		$count[$b]+=$c[$b];
		$total+=$c[$b];
	}
}
arsort($count); //print_r($count);
$tp=0;
foreach ($count as $b=>$c){
	$percent = ($c/$total); $tp+=$percent;
	echo "Landed on $b ".sprintf("%.4f",$percent)." of the time\n";
}
echo "Total percent: ".sprintf("%.8f",$tp);
